...
export function is_fraction_like(target: any): target is IFractionLike
{
    return  (typeof target === 'object') &&
            ...
            ('denominator' in target && sup.number.is_number(target.denominator)) ;
}
...
export class Fraction implements IFractionLike
{
    ...
    private constructor(numerator: IFractionLike | number , denominator?: number) 
    {
        ...
        if(sup.number.is_number(numerator))
        {
    ...
    public static create(numerator: IFractionLike | number, denominator: boolean | number = 1, short_cut: boolean = true): Fraction
    {
        ...
        if(is_fraction_like(numerator))
        {
            ...
            //@ts-ignore
            if(sup.other.is_boolean(denominator)) ret.short_cut(denominator);
            else ret.short_cut();
        }
        else 
        {
    ...
    public static create_from_decimal(decimal: number): IFractionLike
    {
        ...
        const common_factor = sup.math.gcd(numerator, denominator);
    ...
    public s_add(target: Fraction, short_cut: boolean = true)    
    {
    ...
    public add(target: Fraction, ...targets: Fraction[])
    {
        return this.quick_math(this.s_add.bind(this), target, targets);
    }

    public s_sub(target: Fraction, short_cut: boolean = true)
    {
        let ret = target.clone();
        this.quick_reduce_to_common_denominator(ret, short_cut);
    ...
    public sub(target: Fraction, ...targets: Fraction[])
    {
        return this.quick_math(this.s_sub.bind(this), target, targets);
    }

    public s_multiple(target: Fraction, short_cut: boolean = true)
    {
        let ret = target.clone();
        if(short_cut)
        {
    ...
    public multiple(target: Fraction, ...targets: Fraction[])
    {
        return this.quick_math(this.s_multiple.bind(this), target, targets);
    }

    public s_divide(target: Fraction, short_cut: boolean = true)
    {
        let ret = target.reverse();
        return this.s_multiple(ret, short_cut);
    }

    public divide(target: Fraction, ...targets: Fraction[])
    {
 ...
 */
    ...
    quick_reduce_to_common_denominator(target: Fraction, short_cut: boolean)
    {
        ...
        let lcm = sup.math.lcm(this.denominator, target.denominator);
